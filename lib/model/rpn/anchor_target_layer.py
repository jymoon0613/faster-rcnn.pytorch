...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        # labels 생성 (B, n) (-1로 채움)
        # bbox_inside_weights 생성 (B, n) (0으로 채움)
        # bbox_outside_weights 생성 (B, n) (0으로 채움)
        labels = gt_boxes.new(batch_size, inds_inside.size(0)).fill_(-1)
        bbox_inside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_()
        bbox_outside_weights = gt_boxes.new(batch_size, inds_inside.size(0)).zero_()

        # n개의 anchor boxes와 gt_boxes 간의 IoU를 계산
        ...
        # argmax_overlaps = (B, n) anchor box indices
        ...
        if not cfg.TRAIN.RPN_CLOBBER_POSITIVES: # cfg.TRAIN.RPN_CLOBBER_POSITIVES = False
            labels[max_overlaps < cfg.TRAIN.RPN_NEGATIVE_OVERLAP] = 0 # cfg.TRAIN.RPN_NEGATIVE_OVERLAP = 0.3
            # 각 anchor box 별로 식별된 가장 IoU가 높은 gt_box에 대해 IoU가 IoU_bg 미만인 경우 negative sample로 사용 (label = 0)

        # 남아 있는 anchor boxes 식별
        ...
        # 각 anchor box 별로 식별된 가장 IoU가 높은 gt_box에 대해 IoU가 IoU_fg 이상인 경우 positive sample로 사용 (label = 1)
        ...
        for i in range(batch_size):
            ...
            if sum_fg[i] > num_fg:
                fg_inds = torch.nonzero(labels[i] == 1).view(-1) # fg의 indices를 식별
...
#           num_bg = cfg.TRAIN.RPN_BATCHSIZE - sum_fg[i]
        ...
        offset = torch.arange(0, batch_size)*gt_boxes.size(1) # batch size offset 지정
        # offset = [0, 1, ..., B] * 20 = [0, 20, ..., 20B]
