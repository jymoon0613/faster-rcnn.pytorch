...
def nms_cpu(dets, thresh):
    ...
    # ! dets   = (12000, 5) 
    # ! thresh = 0.7
    # ! 현재 예시에서는 dets[4]가 해당 proposal의 objectness score를 의미함
    ...
    # ! proposal의 넓이 계산 
    areas = (x2 - x1 + 1) * (y2 - y1 + 1) # ! (12000,)

    # ! 현재 nms 함수 예시에서는 objectness scores가 정렬되어 있다고 가정하지 않음. 따라서 score를 정렬
    order = scores.argsort()[::-1] 
    # ! order = (12000,) -> objectness scores를 기준으로 내림차순으로 정렬했을 때의 proposal indices 
    ...
    while order.size > 0:
        ...
        # ! 이때 np.maximum은 변수 x1[i]와 리스트 x1[order[1:]]에 대해 수행되며, 리스트 x1[order[1:]]의 각 요소는 변수 x1[i]와 비교했을 때 더 큰 값으로 유지/변경됨
        ...
        order = order[inds + 1] # ! index offset 반영
